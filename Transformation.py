...
def compute_transformations(image_path):
    ...
    if contours:
        ...
        for idx in indices:
            cv2.circle(landmarks, tuple(largest[idx][0]), 5, (255, 0, 0),
                       -1)
        for cnt in contours:
            area = cv2.contourArea(cnt)
        ...
        if M_leaf['m00'] != 0:
            ...
            x, y, bw, bh = cv2.boundingRect(largest)
            for i in range(5):
                cv2.circle(landmarks, (cx_leaf, y + int(bh * i / 4)), 5,
                           (0, 255, 0), -1)
...
def draw_figure(fig, results):
    ...
    show(fig.add_subplot(gs[1, 0]), analyze,
         "Figure IV.5: Analyze object")
    show(fig.add_subplot(gs[1, 1]), landmarks,
         "Figure IV.6: Pseudolandmarks")

    ax_hist = fig.add_subplot(gs[1, 2:])
    for name, (x, y, color) in build_color_histogram(img,
                                                     mask_binary).items():
